``My Query`` -> Once the certificate is verified, the client encrypts it's synochronous encryption key using the public key of the server, which can only be decrypted by the servers private key right?
